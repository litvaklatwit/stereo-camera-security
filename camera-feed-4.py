...
def inside_box(values, box):
    return values[(values[:, 0] > box[0]) & (values[:, 0] < box[2])
            & (values[:, 1] > box[1]) & (values[:, 1] < box[3])]
...
regions = [np.array([400, 240, 600, 400, 8, 12])]
...
while True:
    ...
    for i, c in enumerate(cnts):
        ...
        for region in regions:
            olap = box_overlap(bounding_rect, region[:4])
            if olap[2] - olap[0] > 0 and olap[3] - olap[1] > 0:
                x_center = (olap[2] + olap[0]) // 2
                y_center = (olap[3] + olap[1]) // 2

                olap_pts = points[olap[1]:olap[3], olap[0]:olap[2]]
                olap_img = filtered_img[olap[1]:olap[3], olap[0]:olap[2]]
                olap_thresh = thresh[olap[1]:olap[3], olap[0]:olap[2]]

                olap_pts = olap_pts[(olap_thresh > 0) & (olap_img > filtered_img.min()) & (~np.isnan(olap_pts[:, :, 2]))
                        & (~np.isinf(olap_pts[:, :, 2]))]
                olap_pts = olap_pts[:, 2]
                olap_pts = reject_outliers(olap_pts, m=2)
                #print(olap_pts[np.isnan(olap_pts)].shape)
                #olap_pts = points[(thresh > 0) & (filtered_img > filtered_img.min()) & ~np.isnan(points[:, :, 2])
                #        & (~np.isinf(points[:, :, 2]))]
                #print(olap_pts.shape)
                #print(np.min(olap_pts[:, 0]))
                #olap_pts = inside_box(olap_pts, olap)[2].ravel()
                #olap_pts = olap_pts[~np.isnan(olap_pts)]
                #olap_pts = olap_pts[olap_pts > filtered_img.min()]
                #olap_pts = reject_outliers(olap_pts, m=3)

                depth = np.mean(olap_pts)
                depth = depth_of(-depth) # TODO: UNCOMMENT WHEN I AM DONE CALIBRATING
                
                #depth = points[y_center, x_center, 2]
                #depth = depth_of(-depth)
                #print(str(depth) + "\n" + str(depth_of(-depth)))
                depth_str = str(depth)# + "\n" + str(depth_of(-depth))
                
                color = (0, 0, 255) if depth > region[4] and depth < region[5] else (0, 255, 0)

                cv2.rectangle(frameL, tuple(olap[:2]), tuple(olap[2:]), color, 2)
                cv2.putText(frameL, depth_str, (x_center, y_center),
